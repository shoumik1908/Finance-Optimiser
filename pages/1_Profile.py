"""
Profile — Multi-step wizard (White + Purple theme)
"""
import streamlit as st
from engine import optimise_finances, project_finances, generate_summary
from ui import inject_css, PURPLE, PURPLE_BG, TEXT, TEXT_SEC, BG, CARD
...
<div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid #E5E7EB; margin-bottom:24px;">
    ...
    <div style="display:flex; gap:8px;">
        ...
        <a href="/Profile" target="_self" style="font-family:Inter,sans-serif; font-size:0.85rem; color:{PURPLE}; text-decoration:none; padding:6px 14px; border-radius:8px; background:{PURPLE_BG};">Profile</a>
...
st.markdown(f'<p style="font-family:Inter,sans-serif; color:{TEXT_SEC}; font-size:0.85rem;">Step {step} of 5</p>', unsafe_allow_html=True)

# Step 1: Income & Expenses
if step == 1:
    st.markdown("### 💵 Income & Expenses")
    income = st.number_input("Monthly Income (₹)", min_value=0, value=st.session_state.get("income", 50000), step=1000)
    expenses = st.number_input("Monthly Fixed Expenses (₹)", min_value=0, value=st.session_state.get("expenses", 20000), step=1000)
...
# Step 2: Assets
    ...
    num_assets = st.number_input("How many assets?", 0, 5, st.session_state.get("num_assets", 1))
    assets = []
    for i in range(num_assets):
        ...
        with col2:
            a_amount = st.number_input(f"Value (₹)", 0, 10000000, 100000, key=f"aamt{i}")
        with col3:
            a_return = st.number_input(f"Annual Return (%)", 0.0, 30.0, 6.0, key=f"aret{i}")
...
# Step 3: Debts
    ...
    st.markdown("### 💳 Debts")
    num_debts = st.number_input("How many debts?", 0, 5, st.session_state.get("num_debts", 1))
    debts = []
...
# Step 4: Goals
    ...
    num_goals = st.number_input("How many goals?", 0, 5, st.session_state.get("num_goals", 1))
    goals = []
    for i in range(num_goals):
        ...
        with col2:
            g_amount = st.number_input(f"Target (₹)", 0, 10000000, 500000, key=f"gamt{i}")
        with col3:
