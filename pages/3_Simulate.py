"""
Simulate — Before/after comparison (White + Purple theme)
"""
import streamlit as st
import plotly.graph_objects as go
from engine import optimise_finances, project_finances, generate_summary, replan
from ui import inject_css, allocation_stacked_bar, PURPLE, PURPLE_BG, TEXT, TEXT_SEC, GREEN, RED, TEAL, CARD
...
<div style="display:flex; justify-content:space-between; align-items:center; padding:12px 0; border-bottom:1px solid #E5E7EB; margin-bottom:24px;">
    ...
    <div style="display:flex; gap:8px;">
        ...
        <a href="/Simulate" target="_self" style="font-family:Inter,sans-serif; font-size:0.85rem; color:{PURPLE}; text-decoration:none; padding:6px 14px; border-radius:8px; background:{PURPLE_BG};">Simulate</a>
...
if "profile" not in st.session_state:
    ...
    <div style="text-align:center; padding:80px 0;">
        ...
        <p style="color:{TEXT_SEC};">Build your financial profile first to simulate life events.</p>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f'<p style="color:{TEXT_SEC}; font-size:0.9rem; margin-bottom:24px;">See how your plan adapts when conditions change.</p>', unsafe_allow_html=True)
    ...
    if st.button("🔄 Run Simulation", type="primary", use_container_width=True):
        ...
        with col_before:
            st.markdown(f'<h4 style="color:{TEXT_SEC};">Original Plan</h4>', unsafe_allow_html=True)
            final_nw_orig = orig_proj["net_worth"][-1] if orig_proj["net_worth"] else 0
            st.markdown(f'<p style="font-family:JetBrains Mono,monospace; font-size:2rem; color:{TEXT};">₹{final_nw_orig:,.0f}</p><p style="color:{TEXT_SEC}; font-size:0.8rem;">Projected Net Worth</p>', unsafe_allow_html=True)
            allocation_stacked_bar(orig_alloc)
        with col_after:
            st.markdown(f'<h4 style="color:{TEXT_SEC};">Revised Plan</h4>', unsafe_allow_html=True)
            final_nw_new = new_proj["net_worth"][-1] if new_proj["net_worth"] else 0
            delta = final_nw_new - final_nw_orig
            delta_color = GREEN if delta >= 0 else RED
            st.markdown(f'<p style="font-family:JetBrains Mono,monospace; font-size:2rem; color:{PURPLE};">₹{final_nw_new:,.0f}</p><p style="color:{delta_color}; font-size:0.8rem;">₹{delta:+,.0f} vs original</p>', unsafe_allow_html=True)
            allocation_stacked_bar(new_alloc)

        # Combined chart
        st.markdown("#### Net Worth: Original vs Revised")
        fig = go.Figure()
        fig.add_trace(go.Scatter(x=orig_proj["months"], y=orig_proj["net_worth"], name="Original Plan", line=dict(color="#D1D5DB", width=2, dash="dot")))
        ...
        fig.add_trace(go.Scatter(x=combined_months, y=combined_nw, name="Revised Plan", line=dict(color=PURPLE, width=2)))
        fig.add_vline(x=event_month, line_dash="dot", line_color=RED, annotation_text=f"Event (month {event_month})", annotation_font_color=RED)
        fig.update_layout(
        ...
        for i, (label, orig_val, new_val) in enumerate([
            ...
            d = new_val - orig_val
            cols[i].metric(label, f"₹{new_val:,.0f}", f"₹{d:+,.0f}")
